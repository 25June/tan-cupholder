...
export const Footer = () => {
  return (
    <footer className="text-[16px] text-white leading-[24px] grid sm:grid-cols-2 grid-cols-1 justify-between bg-logo-orange-border sm:px-8 px-4 py-[40px] sm:py-[64px]">
      <motion.div
      ...
      >
        <motion.p
          ...
          className="md:block sm:hidden block font-semibold text-white text-[16px] leading-[30px]"
        >
          Tìm Hiểu Thêm
        </motion.p>
        <motion.div
          ...
          className="grid md:grid-cols-3 sm:grid-cols-1 grid-cols-2 gap-4 md:self-start sm:self-end opacity-70"
        >
          <motion.p
            viewport={{ once: true }}
            variants={durationRight}
            className="md:hidden sm:block hidden font-semibold text-white text-[16px] leading-[30px]"
          >
            Tìm Hiểu Thêm
          </motion.p>
          {menus.map((menu, index) => (
            <motion.div
              key={index}
              viewport={{ once: true }}
              className="flex flex-col sm:gap-3 gap-5 opacity-70 opacity-70"
            >
              {menu.map((item) => (
                <motion.a
                  target="_self"
                  ...
                  className="text-[16px] w-fit leading-[24px] hover:text-white"
                  key={item.name}
                >
                  {item.name}
                </motion.a>
          ...
          <motion.div
            viewport={{ once: true }}
            variants={durationRight}
            className="md:flex hidden flex-col sm:gap-3 gap-5 !opacity-50"
          >
            <p>Copyright © {new Date().getFullYear()} TaN</p>
            <span>All rights reserved</span>
          </motion.div>
        ...
        <motion.div
          viewport={{ once: true }}
          variants={durationLeft}
          className="sm:hidden flex flex-col gap-5 !opacity-50"
        >
          <p>Copyright © {new Date().getFullYear()} TaN</p>
          <span>All rights reserved</span>
        </motion.div>
