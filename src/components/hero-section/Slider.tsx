    ...
    <>
      {imgOrders.map((img, index) => {
        ...
        return (
          <div
            key={img}
            className={`w-4/5 h-4/5 max-w-md max-h-128 absolute transition-all duration-700 drop-shadow-lg overflow-hidden rounded-xl ${tranformY} ${zIndex[transitionIndex]} ${tranformX} ${contrast[index]} ${o}`}
          >
            <EditableImage
              imageKey={img}
              src={''}
